...
<div class="bg-white m-10 rounded-xl drop-shadow-xl">
    ...
    <div class="flex justify-center">
      <button class="m-6 p-2 bg-amber-400 text-white rounded-md" on:click={() => window.location.href = "/login"}>Login</button>
    </div>

    <p class="flex justify-center text-stone-500 text-lg">No account yet? Wanna make one?</p>

    <div class="flex justify-center">
      <button class="m-6 p-2 bg-amber-400 text-white rounded-md" on:click={() => window.location.href = "/create_account"}>Create Account</button>
    </div>
</div>